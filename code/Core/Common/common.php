 ...
 */
 ...
 // 这里以后考虑到加载到  maxhom.php 中这个文件是我们自己写的函数，和系统函数没有关系
 // 1. execl 导入和导出  add by dengweicai
//读取execl文件内容  这两个函数还需要测试
function readExeclContent($filePath){
	//echo $filePath;exit;
	//$filePath = "./Uploads/daoru/sjy.xls";
	...
	for ($row = 1; $row <= $highestRow; $row++){//行数是以第1行开始
		for ($column = 'A'; $column <= $highestColumm; $column++) {//列数是以A列开始
			...
			if(!empty($cell))$dataset[$row][$column]=strval($cell);
		}
	}
...
// 导出格式
/*****
 *  list 数组分三个部分， 一个header  一个data 一个是文件名称(选填的)
*  $list = array('header'=>array('序号','项目名称'),'data'=>array('id'=>'1','name'=>'有个房'),'filename'=>'xxx.xls')
*****/
function exportExecl($list)
{
	//$list = array('header'=>array('序号','项目名称'),'data'=>array(0=>array('id'=>'1','name'=>'有个房'),1=>array('id'=>'2','name'=>'名望角')));
	//编码，防止乱码
	$excelname=iconv('utf-8',"gb2312//IGNORE", '$obj_phpexcel');

	include_once(APP_PATH.'/Lib/Excel/PHPExcel/Writer/IWriter.php') ;
	include_once(APP_PATH.'/Lib/Excel/PHPExcel/Writer/Excel5.php') ;
	include_once(APP_PATH.'/Lib/Excel/PHPExcel/Classes/PHPExcel.php') ;
	include_once(APP_PATH.'/Lib/Excel/PHPExcel/IOFactory.php');
	$obj_phpexcel = new PHPExcel();

	$n = ord("a");//表示小写字母a的ASCLL码(阿斯克码);
	foreach($list['header'] as  $vv)
	{
		//?CHRTRANC($n,"c1","");//去掉字段C1
		//下面两行代码是得出Excel表以A,B,C...下的为1行的$Key值;
		$obj_phpexcel->getActiveSheet()->setCellValue(chr($n).'1',"$vv");
		$n++;
	}


	$t = 2;
	foreach($list['data'] as  $vvv)
	{
		$n = ord("A");//表示小写字母A的ASCLL码(阿斯克码);
		foreach($vvv as $k=>$rs){
			$num=$k+2;
			...
			$obj_phpexcel->setActiveSheetIndex(0)->setCellValue(chr($n)."$t", "$rs");
			// echo chr($n)."$t <br />";
			$n++;
		}
		$t++;
	...
	$filename =($list['filename']) ? $list['filename'] : "outexcel.xls";
