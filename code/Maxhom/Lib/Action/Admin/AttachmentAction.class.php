...
class AttachmentAction extends  Action {
		...
		if(!$this->userid){
			if($this->isadmin){
				...
				if(cookie('auth')){
					...
					$maxhom_auth_key = sysmd5($this->sysConfig['ADMIN_ACCESS'].$HTTP_USER_AGENT);
					list($userid, $groupid ,$password) = explode("-", authcode(cookie('auth'), 'DECODE', $maxhom_auth_key));
					$this->userid = $userid;
	...
	public function index(){
		...
		$enupsetup = authcode($auth, 'DECODE', $maxhom_auth_key);
