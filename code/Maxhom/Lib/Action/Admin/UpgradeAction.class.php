...
class UpgradeAction extends AdminbaseAction {
	...
	private $_filearr = array('Maxhom', 'Public', 'Core', '');
	private $_url = 'http://download.maxhom.cn/';
		...
		if(!empty($_GET['do'])) {
			...
			foreach($patchlist as $k=>$v) {		 
				...
				if($this->copyfileerror){
					...
					if(file_exists($upgradezip_source_path.'/maxhom.sql')){
						$sqldata = file_get_contents($upgradezip_source_path.'/maxhom.sql');
	...
	public function checkfile() {
			...
			$v=explode(' ',VERSION);
			$var=$v[0];
			$maxhom_filemd5 = @file_get_contents($this->_url.$var.'/md5file/'.UPDATETIME.".php");
			$maxhom_filemd5_arr = json_decode($maxhom_filemd5, 1);unset($maxhom_filemd5);

			$filemd5_arr_key = array_keys($this->filemd5_arr);
			$maxhom_filemd5_arr_key = array_keys($maxhom_filemd5_arr);

			$editfiles = array_diff($maxhom_filemd5_arr, $this->filemd5_arr);
			$lostfile = array_diff( $maxhom_filemd5_arr_key,$filemd5_arr_key);
			$unknowfile = array_diff($filemd5_arr_key, $maxhom_filemd5_arr_key);
