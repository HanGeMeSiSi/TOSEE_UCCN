...
class Form extends Think {
	...
	public function catid($info,$value){
		...
		foreach ($Category as $r){
				...
				// add by dengweicai 这里是为了其他的管理员可以访问而修改的
				...
				//if(  !in_array($_SESSION['groupid'],$postgroup) ||  (!in_array( cookie('groupid'),$postgroup)) ) continue;
	...
	public function title($info,$value){
		...
		$maxhom_auth_key = sysmd5(C('ADMIN_ACCESS').$_SERVER['HTTP_USER_AGENT']);
	...
	public function text($info,$value,$isreadonly){
        ...
        if($isreadonly&&ACTION_NAME=='edit'){
		...
		$parseStr.='&nbsp;<font color="red">'.$info['remark']."</font>";
		return $parseStr;
	}
	...
	public function number($info,$value,$isreadonly){
		$info['setup']=is_array($info['setup']) ? $info['setup'] : string2array($info['setup']);
		$id = $field = $info['field'];
	    $validate = getvalidate($info);
	...
	public function textarea($info,$value,$isreadonly){
		$info['setup']=is_array($info['setup']) ? $info['setup'] : string2array($info['setup']);
		$id = $field = $info['field'];
        $validate = getvalidate($info);
        if(ACTION_NAME=='add'){
	...
	public function radio($info,$value){
       ...
       $info['setup']=is_array($info['setup']) ? $info['setup'] : string2array($info['setup']);
        ...
        foreach($optionsarr as $key=>$r) {
			...
			if($labelwidth) $parseStr .= '<label style="float:left;width:'.$labelwidth.'px" class="checkbox_'.$id.'" >';
			$parseStr .= '<input type="radio" class="input_radio '.$info['class'].'" name="'.$field.'" id="'.$id.'_'.$i.'" '.$checked.$onclick.' value="'.$key.'" '.$validate.'>'
					
			.'<lable  id="my'.$id.'_'.$i.'" >'.$r." </lable>";
			if($labelwidth) $parseStr .= '</label>';
            $i++;
		}
		$parseStr.='&nbsp;<font color="red">'.$info['remark']."</font>";
	...
	public function editor($info,$value){
		...
		$attach_auth = authcode("$this->isadmin-1-0-$alowuploadlimit-$Config[attach_allowext]-$file_size-$moduleid", 'ENCODE',$maxhom_auth_key);
		$maxhom_auth = authcode("$this->isadmin-1-0-$alowuploadlimit-$alowuploadexts-$file_size-$moduleid", 'ENCODE',$maxhom_auth_key);

		$str ='';
		$str .= '<div class="editor_box"><div style="display:none;" id="'.$field.'_aid_box"></div><textarea name="'.$field.'" class="'.$info['class'].' width:85%"  id="'.$id.'"  boxid="'.$id.'" '.$validate.'  style="width:99%;height:'.$height.'px;visibility:hidden;">'.$value.'</textarea>';

		$show_page =  $show_page ?  1 :  0;
		//$info['setup']['edittype']='kindeditor';
	
		// add by dengweicai 
		if($info['setup']['edittype']=='UEditor'){ //百度编辑器
			if($toolbar == 'basic') {
			...
			$str .="<script type=\"text/javascript\" src=\"".__ROOT__."/Public/UEditor/ueditor.all.js\"></script>";
			
			$str .= "<script type=\"text/javascript\">\r\n";
			$str .= "var UEditor = UE.getEditor('$textareaid',{serverUrl: ''}); ";
			//$str .= "});\r\n";
			$str .= '</script>';	
			...
			$str .= '</div></div>';
		}else if($info['setup']['edittype']=='ckeditor'){ // add by dengweicai  end 
			if($toolbar == 'basic') {
				$toolbar = "['Bold', 'Italic', '-', 'NumberedList', 'BulletedList', '-', 'Link', 'Unlink' ]\r\n";
			} elseif($toolbar == 'full') {
				$toolbar = "['Source',";
			...
			$str .= "height:{$height},isadmin:$this->isadmin,pages:$show_page,flashupload:$flashupload,textareaid:'".$textareaid."',iframeid:'swfupload',file_limit:$alowuploadlimit,file_types:'".$alowuploadexts."',moduleid:$moduleid,auth:'".$maxhom_auth."',file_size:$file_size,yesdo:'insert2editor',nodo:'nodo',\r\n";
			$str .= "toolbar :\r\n";
			$str .= "[\r\n";
			$str .= $toolbar;
			$str .= "]\r\n";
			...
			if($flashupload){
				...
				$str .=",upImgUrl:\"!".__ROOT__."/index.php?g=Admin&m=Attachment&a=index&isadmin=$this->isadmin&more=1&isthumb=0&file_limit=$alowuploadlimit&file_types=$alowuploadexts&file_size=$file_size&moduleid=$moduleid&auth=$maxhom_auth&editorid=$id&immediate=1&l=$this->lang\"";
				
				$maxhom_auth = authcode("$this->isadmin-1-0-$alowuploadlimit-swf-$file_size-$moduleid", 'ENCODE',$maxhom_auth_key);
				$str .=",upFlashUrl:\"!".__ROOT__."/index.php?g=Admin&m=Attachment&a=index&isadmin=$this->isadmin&more=1&isthumb=0&file_limit=$alowuploadlimit&file_types=swf&file_size=$file_size&moduleid=$moduleid&auth=$maxhom_auth&editorid=$id&immediate=1&l=$this->lang\"";

				$maxhom_auth = authcode("$this->isadmin-1-0-$alowuploadlimit-mpg,wmv,avi,wma,mp3,mid,asf,rm-$file_size-$moduleid", 'ENCODE',$maxhom_auth_key);
				$str .=",upMediaUrl:\"!".__ROOT__."/index.php?g=Admin&m=Attachment&a=index&isadmin=$this->isadmin&more=1&isthumb=0&file_limit=$alowuploadlimit&file_types=mpg,wmv,avi,wma,mp3,mid,asf,rm&file_size=$file_size&moduleid=$moduleid&auth=$maxhom_auth&editorid=$id&immediate=1&l=$this->lang\"";
			...
			$maxhom_auth = authcode("$this->isadmin-1-0-1-gif,jpg,jpeg,png,bmp-$file_size-$moduleid", 'ENCODE',$maxhom_auth_key);
			$upImgUrl =__ROOT__."/index.php?g=Admin&m=Attachment&a=index&isadmin=$this->isadmin&more=1&isthumb=0&file_limit=1&file_types=gif,jpg,jpeg,png,bmp&file_size=$file_size&moduleid=$moduleid&auth=$maxhom_auth&l=$this->lang";
				
			$maxhom_auth = authcode("$this->isadmin-1-0-1-swf,flv-$file_size-$moduleid", 'ENCODE',$maxhom_auth_key);
			$upFlashUrl=__ROOT__."/index.php?g=Admin&m=Attachment&a=index&isadmin=$this->isadmin&more=1&isthumb=0&file_limit=1&file_types=swf,flv&file_size=$file_size&moduleid=$moduleid&auth=$maxhom_auth&l=$this->lang";

			$maxhom_auth = authcode("$this->isadmin-1-0-1-mpg,wmv,avi,wma,mp3,mid,asf,rm,rmvb,wav,wma,mp4-$file_size-$moduleid", 'ENCODE',$maxhom_auth_key);
			$upMediaUrl=__ROOT__."/index.php?g=Admin&m=Attachment&a=index&isadmin=$this->isadmin&more=1&isthumb=0&file_limit=1&file_types=mpg,wmv,avi,wma,mp3,mid,asf,rm,rmvb,wav,wma,mp4&file_size=$file_size&moduleid=$moduleid&auth=$maxhom_auth&l=$this->lang";
	...
	public function images($info,$value){
		...
		$maxhom_auth_key = sysmd5(C('ADMIN_ACCESS').$_SERVER['HTTP_USER_AGENT']);
		$maxhom_auth = authcode($this->isadmin.'-'.$info['setup']['more'].'-0-'.$info['setup']['upload_maxnum'].'-'.$info['setup']['upload_allowext'].'-'.$info['setup']['upload_maxsize'].'-'.$info['moduleid'], 'ENCODE',$maxhom_auth_key);
		...
		<div class="c"></div>
		<input type="button" class="button" value="'.L('upload_images').'" onclick="javascript:swfupload(\''.$field.'_uploadfile\',\''.$field.'\',\''.L('uploadfiles').'\','.$this->isadmin.','.$info['setup']['more'].',0,'.$info['setup']['upload_maxnum'].',\''.$info['setup']['upload_allowext'].'\','.$info['setup']['upload_maxsize'].','.$info['moduleid'].',\''.$maxhom_auth.'\',up_images,nodo)">  ';
	...
	public function files($info,$value){
        ...
        <legend>'.L('upload_files').'</legend><center><div>'.L('upload_maxfiles').' <font color=\'red\'>'.$info['setup']['upload_maxnum'].'</font> '.L('zhang').'</div></center>
		<div id="'.$field.'_images" class="imagesList">'.$data.'</div>
		</fieldset>
		<input type="button"  style="margin-left:5px;" class="button" value="'.L('upload_files').'" onclick="javascript:swfupload(\''.$field.'_uploadfile\',\''.$field.'\',\''.L('uploadfiles').'\','.$this->isadmin.','.$info['setup']['more'].',0,'.$info['setup']['upload_maxnum'].',\''.$info['setup']['upload_allowext'].'\','.$info['setup']['upload_maxsize'].','.$info['moduleid'].',\''.$maxhom_auth.'\',up_images,nodo)">  ';

		return $parseStr;
	}
	
	public function otable($info,$value,$isreadonly){ //select
		$info['setup']=is_array($info['setup']) ? $info['setup'] : string2array($info['setup']);
		$id = $field = $info['field'];
	    $validate = getvalidate($info);
        if(ACTION_NAME=='add'){
		...
		$s1 = $info['setup']['fname1'];
		$s2 = $info['setup']['fname2'];
		$module=$info['setup']['tablename'];
		if($module=='area'){
			$list = M($info['setup']['tablename'])->field($s1.','.$s2)->where("status=1")->select();
		}else{
			$list = M($info['setup']['tablename'])->field($s1.','.$s2)->where("status=1")->select();
		}
		...
		foreach($list as $rs){
			$s = ($rs[$s1] == $value)? "selected='selected'": '';
			$select .= '<option value="'.($rs[$s1]).'" '.$s.'>'.($rs[$s2]).'</option>';
		}
		    $select .= "</select>";
		}
		return $select;
